...
object Sync {
    ...
    fun shouldSync(key: String, window: Int = 4, unit: TimeUnit = TimeUnit.HOURS): Boolean {
        ...
        if (currentSavedValue.isNullOrEmpty()) //Operation has never run or Sync doesn't know about it
            return syncIt(key)
        ...
        return if (DateTime.now() >= syncBlock) syncIt(key) else DONT_SYNC
    }

    /**
     * Tell Snyc that the sync operation was a success.
     *
    ...
    /**
     * Tell Snyc that the sync operation was a failure.
     *
